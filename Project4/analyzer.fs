...
module analyzer =
  ...
  let rec private vardecl tokens = 
    ...
    if next_token = "int" then
      ...
      let next_token = List.head T2
      let str = next_token.Substring(11)
  ...
  let rec private stmt tokens symboltable = 
    ...
    if next_token = ";" then
      ...
      let (T2, t) = vardecl tokens
      if checkList symboltable t then
        (T2, t::symboltable)
      else 
        failwith("redefinition of variable '" + (fst t) + "'")
  ...
  let rec private morestmts tokens symboltable = 
    ...
    if next_token = ";"    ||
       ...
       next_token = "if" then
      //
      let (T2, symboltable2) = stmt tokens symboltable
      let (T3, symboltable3) = morestmts T2 symboltable2
      (T3, symboltable3)
    else 
